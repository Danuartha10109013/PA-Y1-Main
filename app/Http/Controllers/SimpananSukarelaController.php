...
class SimpananSukarelaController extends Controller
{
    ...
    {
        $ids = SimpananSukarela::where('no_simpanan',$id)->value('id'); 
        try {
            Log::info("Menampilkan detail simpanan sukarela dengan ID: $ids");
            $simpanan = SimpananSukarela::with('user')->findOrFail($ids);
            $title = 'Data Simpanan Sukarela';
            // dd($simpanan);
    ...
    public function createsimpanansukarela()
    {
        ...
        $lastId = SimpananSukarela::where('user_id', $userId)
                    ->orderBy('id', 'desc')
                    ->value('id');

        $simpanan_id = $lastId ? $lastId + 1 : 1;
